...
(() => {
  ...
  assert.ok(scheduling.includes('approved_student_ids'), "Teachers must be able to select a live-class roster.");
  assert.ok(scheduling.includes('.eq("class_id", classId)'), "Scheduling must reject selected students outside the teacher's selected class.");
  assert.ok(scheduling.includes('notificationRows'), "Scheduling must notify every selected student.");
