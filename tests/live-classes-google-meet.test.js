...
(() => {
  ...
  assert.ok(schedule.includes('eligibleStudents.map'), 'scheduling must authorize every student in the selected class');
  assert.ok(schedule.includes('notificationRows'), 'scheduling must notify every student in the selected class');
  ...
  assert.ok(!moduleCode.includes('approved_student_ids'), 'teacher form must not require manually selected students');
  ...
  assert.ok(schedule.includes('.eq("class_id", classId)'), 'schedule endpoint must select students only from the selected class');
