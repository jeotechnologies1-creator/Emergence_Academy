...
(() => {
  ...
  assert.ok(join.includes('Live-class attendance recording failed'), 'an attendance write failure must not block an authorized student from joining');
  assert.ok(join.includes('agora_channel_name'), 'Agora room access must use the saved channel identity rather than a legacy meeting URL');
  assert.ok(!join.includes('status,meeting_url'), 'a missing legacy meeting URL must not reject a valid Agora room');
  ...
  assert.ok(moduleCode.includes('enableAudioVolumeIndicator'), 'the classroom should detect active speakers for a modern focused layout');
  assert.ok(moduleCode.includes('MAX_VISIBLE_VIDEO_PARTICIPANTS = 6'), 'the classroom should limit the visible participant frame to six cameras');
  assert.ok(moduleCode.includes('renderVideoGallery'), 'remote cameras should be arranged into an active-speaker stage and participant gallery');
  assert.ok(moduleCode.includes('data-agora-overflow-count'), 'participants beyond the visible frame should be minimized into an overflow indicator');
