...
(() => {
  ...
  assert.ok(options.includes('assignmentPairs'), 'scheduling options must retain only exact teacher class and subject assignments');
  assert.ok(options.includes('student_subjects'), 'scheduling options must load student subject enrolments');
  ...
  assert.ok(moduleCode.includes('enrolledSubjects.includes(subjectId)'), 'student picker must filter by the selected subject');
  assert.ok(schedule.includes('.eq("class_id", classId)'), 'schedule endpoint must reject students outside the selected class');
  assert.ok(schedule.includes('.eq("subject_id", subjectId)'), 'schedule endpoint must reject students outside the selected subject');
