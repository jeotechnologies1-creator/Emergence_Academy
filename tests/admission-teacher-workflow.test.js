...
(() => {
  ...
  assert.ok(teachers.includes('saveTeacherProfileAssignments'), 'admin teacher creation must verify that selected class/subject assignments were saved');
  assert.ok(read('supabase', 'migrations', '202608250008_save_teacher_profile_assignments.sql').includes('save_teacher_profile_assignments'), 'database must save teacher department and assignments atomically');
