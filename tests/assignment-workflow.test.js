...
(() => {
  ...
  const feedbackMigration = read("supabase", "migrations", "202609100003_add_assignment_feedback_workflow.sql");
  assert.ok(feedbackMigration.includes("submission_teacher_review_own_assignment"), "teachers need a scoped submission-review policy");
  assert.ok(feedbackMigration.includes("protect_assignment_submission_review"), "review fields must be protected from student tampering");
  assert.ok(moduleCode.includes("data-review-form"), "teachers need a feedback return form");
  assert.ok(moduleCode.includes("Awaiting teacher review"), "students need an explicit review state");
