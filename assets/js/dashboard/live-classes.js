...
class LiveClassesModule {
  ...
  static form() {
    ...
    const students = this.state.students.map((student) => `<label data-approved-student data-class-id="${this.safe(student.class_id)}" class="flex items-center gap-2 rounded border p-2 text-sm opacity-50"><input disabled type="checkbox" name="approved_student_ids" value="${this.safe(student.id)}"><span>${this.safe(`${student.profiles?.first_name || ""} ${student.profiles?.last_name || ""}`.trim() || student.profiles?.email || "Student")}</span></label>`).join("") || '<p class="text-sm text-slate-500">No students are available for your assigned classes.</p>';
  ...
  static card(session) {
    ...
    return `<article class="rounded-xl bg-white p-5 shadow"><div class="flex justify-between gap-3"><div><h4 class="font-bold text-slate-800">${this.safe(session.title)}</h4><p class="mt-1 text-sm text-slate-500">${this.safe(session.subject_name || "Subject")}</p></div><span class="rounded-full px-3 py-1 text-xs font-medium ${status === "live" ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-700"}">${this.safe(status === "live" ? "LIVE NOW" : status)}</span></div><p class="mt-3 text-sm text-slate-600"><strong>Teacher:</strong> ${this.safe(`${session.teacher_first_name || ""} ${session.teacher_last_name || ""}`.trim() || "Assigned teacher")}<br><strong>Time:</strong> ${this.safe(this.format(session.starts_at))} – ${this.safe(this.format(session.ends_at))}</p>${session.description ? `<p class="mt-2 text-sm text-slate-600">${this.safe(session.description)}</p>` : ""}<div class="mt-4 flex flex-wrap gap-2"><button data-live-action="join" data-id="${this.safe(session.id)}" ${joinable ? "" : "disabled"} class="rounded-lg bg-blue-600 px-3 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50">${joinable ? "Join Class" : status === "ended" ? "Class Ended" : status === "cancelled" ? "Cancelled" : "Not started"}</button>${teacherControls && ["upcoming", "live"].includes(status) ? `<button data-live-action="${status === "live" ? "end" : "cancel"}" data-id="${this.safe(session.id)}" class="rounded-lg border px-3 py-2 text-sm">${status === "live" ? "End class" : "Cancel"}</button>` : ""}</div></article>`;
  }
  static bind() {
    ...
    form?.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        const result = await API.db.functions.invoke("schedule-live-class", { body: { ...Object.fromEntries(data), approved_student_ids: approvedStudentIds } });
