...
create policy course_announcements_read_enrolled on public.course_announcements for select to authenticated using (
  ...
  or (public.current_user_role()='student' and exists (select 1 from public.students s where s.profile_id=auth.uid() and s.class_id=course_announcements.class_id and (course_announcements.subject_id is null or exists (select 1 from public.student_subjects ss where ss.student_id=s.id and ss.subject_id=course_announcements.subject_id))))
  or (public.current_user_role()='parent' and exists (select 1 from public.parent_students ps join public.parents p on p.id=ps.parent_id join public.students s on s.id=ps.student_id where p.profile_id=auth.uid() and s.class_id=course_announcements.class_id and (course_announcements.subject_id is null or exists (select 1 from public.student_subjects ss where ss.student_id=s.id and ss.subject_id=course_announcements.subject_id))))
);
